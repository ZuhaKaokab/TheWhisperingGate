...
- **Camera Focus System**: `CameraFocusController` and `CameraFocusPoint` for dialogue-driven camera direction.
...
## Recent Updates (Latest Session - 30 Nov 2025)
- **Camera Focus System**: Dialogue-driven camera control for cinematic moments during conversations.
  - `CameraFocusController`: Manages smooth camera transitions to focus points with limited player look
  - `CameraFocusPoint`: Marker component for defining camera look-at targets in scenes
  - New dialogue command: `cam:point_id` to focus camera, `cam:reset` to release
  - Integrated with `DialogueManager.ExecuteCommand()` and `PlayerController`
  - Auto-releases focus when dialogue ends
  - Documentation: `CAMERA-FOCUS-SETUP.md`
